package br.com.dlweb.maternidade_ads.database;
...
import br.com.dlweb.maternidade_ads.medico.Medico;
import br.com.dlweb.maternidade_ads.mae.Mae;
import br.com.dlweb.maternidade_ads.bebe.Bebe;

public class DatabaseHelper extends SQLiteOpenHelper {

    // Definição das variáveis globais
    private static final String DATABASE_NAME = "maternidade"; //arte nos bairros
    private static final String TABLE_MAE = "mae"; //aluno
    private static final String TABLE_MEDICO = "medico"; //professor
    private static final String TABLE_BEBE = "bebe"; //polo
    ...
    private static final String CREATE_TABLE_MAE = "CREATE TABLE " + TABLE_MAE + " (" +
            ...
            "CONSTRAINT fk_bebe_mae FOREIGN KEY (id_bebe) REFERENCES bebe (id))";
    /*TABELA PROFESSOR*/
    ...
    /* Início CRUD Professor (antigo Médico)*/
    ...
    public void getAllMedico (Context context, ListView lv) {
        ...
        String[] columns = {"_id", "nome", "especialidade"};
    ...
    /* Fim CRUD Professor (antigo Médico) */
    /* Início CRUD Aluno (antigo Mãe) */
    public long createMae (Mae m) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put("id_bebe", m.getId_bebe()); //ID POLO
        cv.put("nome", m.getNome());
    ...
    public Mae getByIdMae (int id) {
        SQLiteDatabase db = this.getReadableDatabase();
        String[] columns = {"_id", "id_bebe", "nome", "logradouro", "numero", "bairro", "cidade", "cep", "fixo", "celular", "comercial", "data_nascimento"};
        Cursor data = db.query(TABLE_MAE, columns, "_id = ?", new String[]{String.valueOf(id)}, null, null, null);
        data.moveToFirst();
        Mae m = new Mae();
        m.setId(data.getInt(0));
        m.setId_bebe(data.getInt(1));
        m.setNome(data.getString(2));
        m.setLogradouro(data.getString(3));
        m.setNumero(data.getInt(4));
        m.setBairro(data.getString(5));
        m.setCidade(data.getString(6));
        m.setCep(data.getString(7));
        m.setFixo(data.getString(8));
        m.setCelular(data.getString(9));
        m.setComercial(data.getString(10));
        m.setData_nascimento(data.getString(11));
        data.close();
        db.close();
        return m;
    }
    ...
    public void getAllMae (Context context, ListView lv) {
        SQLiteDatabase db = this.getReadableDatabase();
        // Define as colunas da consulta do SELECT
    ...
    /* Fim CRUD Aluno (antigo Mãe) */

    /* Início CRUD Polo (antigo Bebê) */
    ...
    // Recupera apenas os IDs e nomes dos Polos para o Spinner em ALunos
    public void getAllNameBebe (ArrayList<Integer> listBebeId, ArrayList<String> listBebeName) {
        SQLiteDatabase db = this.getWritableDatabase();
        String[] columns = {"_id", "nome"};
        Cursor data = db.query(TABLE_BEBE, columns, null, null, null,
                null, "nome");
        while (data.moveToNext()) {
            int idColumnIndex = data.getColumnIndex("_id");
            listBebeId.add(Integer.parseInt(data.getString(idColumnIndex)));
            int nameColumnIndex = data.getColumnIndex("nome");
            listBebeName.add(data.getString(nameColumnIndex));
        }
        db.close();
    }
    /* Fim CRUD Polo (antigo Bebê) */
}
